...
class AppAdapter(data: MutableList<AppInfo>) :
    ...
    override fun convert(holder: BaseViewHolder, item: AppInfo) {
        ...
        holder.setText(R.id.version, "${item.versionName} (${item.versionCode})")
        holder.setText(R.id.uid, "UID: ${item.uid}")

        if ((context as PackageViewerActivity).choose) {
            holder.getView<Button>(R.id.detail).setText(R.string.choose)
        }
        holder.getView<Button>(R.id.detail).setOnClickListener {
            // 详情页
            AppInfoDetailDialog(context as PackageViewerActivity, item).alertDialog.show()
        }
    }
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            ...
            override fun publishResults(constraint: CharSequence, results: FilterResults) {
                // 更新搜索结果
                setDiffNewData(results.values as MutableList<AppInfo>)
